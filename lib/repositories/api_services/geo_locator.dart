...
class GeoLocator implements MultiOptionsAPIService<Location> {
  ...
  String lastQuery = '';
  List<Location> _lastQueriedLocations = [];
  ...
  Future<List<Location>> performQuery(Object query) async {
  ...
  static List<Location> _convertResponse(String response) {
    List decodedResponse = json.decode(response);
    return decodedResponse.map((e) => convertJsonToLocation(e)).toList();
  }

  static Location convertJsonToLocation(Map<String, dynamic> locationJson) {
    return Location(
